...
Options[ShowLastMeasure] = {"Period"->24, "Unit"->"Hour"};
...
(* Plot a variable of a CISEI file within a (backward) period 
   ...
   period unit: backward period used in the plot
*)
Options[PlotDataLog] = {"MovingAverage" -> 24 };
PlotDataLog[info_, period_, unit_, OptionsPattern[]] := Module[{mres, res, res2, res3, i, j, t, cal, f, label, data},

    cal = Get["DATASETS/calibration.txt"];

    res = DSLib`TailDataLog[period, unit];

    f[r_] :=  If[StringStartsQ[info, "F"] , 
            Tan[(90 - r[info] + cal[info])*Pi/180]*100, 
            r[info]/ cal[info]];

    res2 = Table[ {r["date"], f[r]}, {r, res}];

    res3 = {};
    i = 1;
    t = Partition[ NestWhileList[ DSLib`NextIndex[res, #, 1, unit] &, 1, # < Length[res] & ], 2, 1];  

    Do[ AppendTo[res3, 
        Thread[{res2[[j[[1]], 1]], 
        Mean@Part[ Part[ res2, j[[1]] ;; j[[2]] ] , All, 2] }]];
    , {j, t}];

    If[ OptionValue["MovingAverage"] > 1,
        mres = {};
        Do[
            data = res3[[All, i]];
            madata = Join[ Table[ Mean@data[[All, 2]] , {OptionValue["MovingAverage"] - 1}], data[[All, 2]]]; 
            AppendTo[mres, Thread[ {data[[All, 1]], MovingAverage[ madata, OptionValue["MovingAverage"]]}] ]; 
        , {i, 1, 3}];
        ,
        mres = Transpose@res3;
    ];
